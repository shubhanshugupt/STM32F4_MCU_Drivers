 ...
 *
 * This program triggers interrupt using a PushButton.
 * LED toggles for every interrupt occurance.
 *
...
int main(void)
{
	...
	GPIO_IRQConfig(IRQ_NO_EXTI0, ENABLE);		//EXTI0 since PIN0 is used for button
...
{
	GPIO_IRQHandling(GPIO_PIN_0);			//We reset the bit in EXTI PR register, since the interrupt is now triggered
